...
export const addItems = async (parent: string, data: Array<IProduct>) => {
  ...
  let priceMax: number = Number(data[0].price);
  let priceMin: number = Number(data[0].price);
  let quantityMin: number = Number(data[0].stock);
  let quantityMax: number = Number(data[0].stock);
  el.innerHTML = '';
